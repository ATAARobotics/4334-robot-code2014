...
public class Preferences {
    ...
    public Object getObject(String key, Object backup) {
        try {
    ...
    public GordianList getArmCalibration(File file) {
        try {
            GordianScope gordian = new GordianScope();
            String contents = TextFiles.getTextFromFile(file);
            gordian.run(contents);
            return (GordianList) gordian.variables().get("list");
        } catch (InternalNotFoundException ex) {
            throw new IllegalStateException("Arm calibration not found");
        }
    }
}
